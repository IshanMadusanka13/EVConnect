...
class BookingListActivity : AppCompatActivity() {
    ...
    private lateinit var btnCreateBooking: FloatingActionButton
    private lateinit var btnRefresh: ImageButton

    private var allBookings = listOf<Booking>()
    private var filteredBookings = listOf<Booking>()

    /** Activity lifecycle: onCreate Initialize views and load bookings */
    /** Activity lifecycle: onCreate Initialize views and load bookings */
    override fun onCreate(savedInstanceState: Bundle?) {
        // CRITICAL: super.onCreate() MUST be called first!
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_booking_list)

        // Now set exception handler (optional, for debugging)
        Thread.setDefaultUncaughtExceptionHandler { thread, throwable ->
            Log.e("BookingListActivity", "═══════════════════════════════════════════════")
            Log.e("BookingListActivity", "UNCAUGHT EXCEPTION!")
            Log.e("BookingListActivity", "Thread: ${thread.name}")
            Log.e("BookingListActivity", "Exception: ${throwable.javaClass.simpleName}")
            Log.e("BookingListActivity", "Message: ${throwable.message}")
            Log.e("BookingListActivity", "Stack trace:", throwable)
            Log.e("BookingListActivity", "═══════════════════════════════════════════════")
        }

        Log.d("BookingListActivity", "onCreate started")
        ...
        btnRefresh.setOnClickListener { loadBookings() }
    }
