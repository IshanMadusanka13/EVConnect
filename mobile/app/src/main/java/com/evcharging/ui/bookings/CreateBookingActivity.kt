...
/**
 * Activity for creating new charging station bookings
 * Implements step-by-step booking process
 */
class CreateBookingActivity : AppCompatActivity() {
    ...
    private fun setupListeners() {
        btnSearchOwner.setOnClickListener {
            searchEVOwner()
        }
        ...
        radioGroupCharger.setOnCheckedChangeListener { _, checkedId ->
            selectedChargerType = when (checkedId) {
                R.id.rbDC -> "DC Fast"
                else -> "AC"
            }
        }
    ...
    private fun validateCurrentStep(): Boolean {
        return when (currentStep) {
            1 -> {
                if (evOwner == null) {
                    Toast.makeText(this, "Please search and select an EV Owner", Toast.LENGTH_SHORT).show()
                    false
                } else true
            }
            2 -> {
            ...
            3 -> {
                when {
                    ...
                    spinnerStartTime.selectedItem.toString() >= spinnerEndTime.selectedItem.toString() -> {
